...
export class BaexElement extends HTMLElement {
  ...
  static get observedAttributes(): string[] {
    if (hasWasm('resolveObservedAttributes')) {
      ...
      return (
        window as unknown as Record<string, (p: unknown) => string[]>
...
// [ANOMALY: NodeMap currently relies on data-baex attributes; should transition to internal references]
  ...
  private _initializeNodeMap(bindings: Binding[]): void {
    ...
    for (const b of bindings) {
      const el = this.querySelector(`[data-baex="${b.marker}"]`) as HTMLElement | null;
      if (el) {
  ...
  private _applyPatches(patches: PropertyPatch[]): void {
    ...
    for (const patch of patches) {
      const binding = Array.from(this._bindingSet.values()).find(
        (b) =>
          (b as any).type === 'property' && (b as any).propName === patch.propName ||
          (b as any).type === 'bool' && (b as any).attrName === patch.propName,
      );

      if (!binding) continue;

      const node = this._nodeMap.get((binding as any).marker);
      if (!node) continue;

      const b = binding as any;
      if (b.type === 'property') {
        (node as any)[b.propName] = patch.value;
      } else if (b.type === 'bool') {
        if (patch.value) {
          node.setAttribute(b.attrName, '');
        } else {
          node.removeAttribute(b.attrName);
        }
      } else if (b.type === 'text') {
        node.textContent = patch.value as string | null;
      }
    }
  }
  ...
  private _applyBindings(bindings: Binding[]): void {
    for (const b of bindings) {
      ...
      if (binding.type === 'event') {
        el.addEventListener(binding.eventName, binding.value);
      } else if (binding.type === 'property') {
        (el as any)[binding.propName] = binding.value;
  ...
  private _serialize(value: unknown, type?: unknown): string | null {
    if (hasWasm('serializeProperty')) {
      const result = (
        window as unknown as Record<string, (v: unknown, t: string | undefined) => unknown>
...
export function property(decl?: PropertyDeclaration): (target: object, key: string | symbol) => void {
...
export function state(decl?: PropertyDeclaration): (target: object, key: string | symbol) => void {
  return (target: object, key: string | symbol) => {
    if (!target) return;
    const ctor = (target as any).constructor || target;
    if (!ctor || typeof ctor !== 'function') return;
