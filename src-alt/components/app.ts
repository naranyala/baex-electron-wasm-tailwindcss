...
const MENU_CATEGORIES = [
  ...
  { id: 'leaflet', name: 'Leaflet Maps', color: 'blue' },
];
...
export class AppElement extends BaexElement {
  ...
  private _initLeafletMap() {
    setTimeout(() => {
      const mapContainer = this.querySelector('#leaflet-map');
      if (mapContainer) {
            ...
            html`
              <div class="max-w-5xl">
                ...
                <div class="p-6 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                  ...
                  <p class="text-sm text-white/50">Select "Accordion Components" or "Leaflet Maps" from the sidebar.</p>
                </div>
              </div>
            `,
                            ...
                            when(
                              ...
                              html`
                                <div class="max-w-4xl">
                                  ...
                                  <div class="text-[1.1rem] font-bold text-white/80 mb-3 pb-2 border-b border-white/10">Implementation</div>
                                  <baex-code-block 
                                    lang="typescript"
                                    code="class AccordionItem extends BaexElement {
                                      @property({ type: String }) title = '';
                                      @property({ type: Boolean }) open = false;

                                      render() {
                                        return html\`
                                          <div class='border border-white/10 rounded-lg mb-2'>
                              ...
                              when(
                                ...
                                html`
                                  <div class="max-w-5xl">
                                    <div class="text-[1.1rem] font-bold text-blue-400 mb-6 pb-2 border-b border-white/10">Leaflet Maps Integration</div>
                                    <div class="grid grid-cols-1 gap-6">
                                      <div class="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                                        <div id="leaflet-map" class="h-[500px] w-full rounded-xl border border-white/10"></div>
                                        <p class="text-sm text-white/50 mt-4">Interactive map rendered via Leaflet.js with OpenStreetMap tiles.</p>
                                      </div>
                                      <div class="text-[1.1rem] font-bold text-white/80 mb-3 pb-2 border-b border-white/10">Implementation</div>
                                      <baex-code-block 
                                        lang="typescript"
