...
import { css, html, type Binding } from '../framework/template.js';
...
describe('html', () => {
  ...
  it('creates an event binding with @ prefix', () => {
    ...
    const b = result.bindings[0] as Extract<Binding, { type: 'event' }>;
  ...
  it('creates a property binding with . prefix', () => {
    ...
    expect(result.bindings).toHaveLength(1);
    const b = result.bindings[0] as Extract<Binding, { type: 'property' }>;
  ...
  it('creates a bool binding with ? prefix', () => {
    ...
    expect(result.bindings).toHaveLength(1);
    const b = result.bindings[0] as Extract<Binding, { type: 'bool' }>;
  ...
  it('handles multiple bindings in one template', () => {
    ...
    `;
    const eventB = result.bindings.find((b) => b.type === 'event') as Extract<Binding, { type: 'event' }>;
    const propB = result.bindings.find((b) => b.type === 'property') as Extract<Binding, { type: 'property' }>;
    const boolB = result.bindings.find((b) => b.type === 'bool') as Extract<Binding, { type: 'bool' }>;
